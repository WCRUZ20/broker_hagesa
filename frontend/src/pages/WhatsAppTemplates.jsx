...
export default function WhatsAppTemplates() {
  ...
  const loadItems = async () => {
    try {
      const res = await API.get("/seguimiento/plantillas-whatsapp");
      setItems(res.data);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (confirm("¿Eliminar plantilla?")) {
      await API.delete(`/seguimiento/plantillas-whatsapp/${id}`);
      loadItems();
    }
  };
  ...
  const handleBulkDelete = () => {
    ...
    Promise.all(selected.map((id) => API.delete(`/seguimiento/plantillas-whatsapp/${id}`))).then(() => {
      ...
      <div className="row mb-4">
        <div className="col-12">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              <h2 className={`mb-1 fw-bold ${darkMode ? 'text-white' : 'text-dark'}`}>Plantillas WhatsApp</h2>
              <p className={`mb-0 ${darkMode ? 'text-muted' : 'text-secondary'}`}>Define mensajes para WhatsApp</p>
            </div>
