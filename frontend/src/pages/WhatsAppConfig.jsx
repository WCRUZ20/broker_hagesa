...
export default function WhatsAppConfig() {
  ...
  const loadItems = async () => {
    try {
      const res = await API.get("/seguimiento/parametrizaciones-whatsapp");
      setItems(res.data);
    } catch (err) {
  ...
  const handleEdit = (cfg) => { setEditing(cfg); setShowModal(true); };
  const handleDelete = async (id) => {
    if (confirm("¿Eliminar configuración?")) {
      await API.delete(`/seguimiento/parametrizaciones-whatsapp/${id}`);
      loadItems();
    }
  };
  const toggleSelect = (id) => { setSelected(prev => prev.includes(id) ? prev.filter(i=>i!==id) : [...prev,id]); };
  const toggleSelectAll = () => { if(selected.length===filtered.length) setSelected([]); else setSelected(filtered.map(c=>c.id)); };
  const handleBulkDelete = () => {
    ...
    Promise.all(selected.map(id=>API.delete(`/seguimiento/parametrizaciones-whatsapp/${id}`))).then(()=>{ setSelected([]); loadItems(); });
  };
  ...
  const renderStatusBadge = (estado) => {
    ...
    return (
      <span className={`badge bg-${color} px-3 py-2 rounded-pill fw-normal d-flex align-items-center gap-1`} style={{ fontSize:'0.875rem', letterSpacing:'0.5px', width:'fit-content' }}>
  ...
  const filtered = items.filter(i => i.ACCOUNT_SID.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="row mb-4">
        <div className="col-12">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              <h2 className={`mb-1 fw-bold ${darkMode ? 'text-white' : 'text-dark'}`}>Config. WhatsApp</h2>
              <p className={`mb-0 ${darkMode ? 'text-muted' : 'text-secondary'}`}>Credenciales de la API</p>
            </div>
            <button className="btn btn-primary px-4 py-2 rounded-3 shadow-sm d-flex align-items-center gap-2" onClick={() => { setEditing(null); setShowModal(true); }} style={{ fontWeight:'500', transition:'all 0.3s ease', border:'none' }}>
              ...
              <div className="row align-items-center">
                ...
                <div className="col-md-3">
                  <div className="position-relative">
                    ...
                    <input type="text" className={`form-control rounded-3 border-0 shadow-sm ps-5 py-2 ${darkMode ? 'bg-secondary text-white' : 'bg-light'}`} placeholder="Buscar" value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} style={{ fontSize:'0.95rem', transition:'all 0.3s ease' }} />
                  </div>
                </div>
              ...
              <table className={`table table-hover align-middle mb-0 ${darkMode ? 'table-dark' : ''}`}>
                <thead className={`${darkMode ? 'border-secondary' : 'bg-light border-0'}`}>
                  <tr style={{ fontSize:'0.9rem', fontWeight:'600' }}>
                    ...
                    <th className="py-3 border-0">SID</th>
                  ...
                  {filtered.map(c=> (
                    <tr key={c.id} className={`${darkMode ? 'border-secondary' : ''}`} style={{ transition:'all 0.2s ease', fontSize:'0.95rem' }}>
                      <td className="ps-4 py-3 border-0">
                        <input type="checkbox" className="form-check-input rounded" checked={selected.includes(c.id)} onChange={()=>toggleSelect(c.id)} style={{ transform:'scale(1.1)' }} />
                      </td>
                      <td className="py-3 border-0">{c.ACCOUNT_SID}</td>
                      <td className="py-3 border-0">{c.FROM_NUMBER}</td>
