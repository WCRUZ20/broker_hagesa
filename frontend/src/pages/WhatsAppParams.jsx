...
export default function WhatsAppParams() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        await API.put(`/seguimiento/parametros-whatsapp/${id}`, form);
      } else {
        const res = await API.post("/seguimiento/parametros-whatsapp", form);
        setId(res.data.id);
      }
  ...
  return (
    <div className="container" style={{ maxWidth: 900 }}>
      <h2 className={`mb-3 fw-bold ${darkMode ? "text-white" : "text-dark"}`}>Parámetros para envíos de WhatsApp</h2>
