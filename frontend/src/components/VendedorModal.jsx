...
export default function VendedorModal({ vendedor, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await API.put(`/vendedores/${vendedor.id}`, form);
      } else {
        ...
        <div className="modal-content">
          <form onSubmit={handleSubmit}>
            <div className="modal-header">
              <h5 className="modal-title">{isEdit ? "Editar Vendedor" : "Nuevo Vendedor"}</h5>
